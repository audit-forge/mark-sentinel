# Arckon by RiskRaven — AI Governance & Human Oversight Policy

## Human Oversight Mechanisms

Arckon by RiskRaven operates as an audit tool, not a decision-making agent. All findings are
surfaced to human operators who make final remediation decisions. Arckon does not autonomously
...
- No automated enforcement actions are performed — Arckon only reads and reports.
- Alert notifications (email, Slack, webhook) inform humans; humans decide next steps.

### Human-in-the-Loop Controls

Arckon enforces human-in-the-loop by design:
...
- All agent actions are logged in `.arckon-server.log` and the dashboard activity feed.
...
Arckon uses AI models (Anthropic Claude, OpenAI GPT) only for:
