...
def build(root: pathlib.Path) -> bytes:
    ...
    prereqs = ("<ul>"
               ...
               "<li>Admin/root for service install</li>"
               "</ul>")

    macos = ("<pre><code>curl -s http://SERVER_IP:7331/bundle.tar.gz | tar -xz -C /tmp\n"
             "python3 /tmp/sentinel/server.py\n"
             "# Dashboard opens at http://localhost:7331\n"
             "curl -s http://SERVER_IP:7331/bundle.tar.gz | tar -xz -C /tmp \n"
             "sudo bash /tmp/sentinel/install.sh --server http://SERVER_IP:7331 --token YOUR_TOKEN\n\n"
             "# Service management:\n"
             "sudo launchctl list | grep sentinel # check status\n"
    ...
    windows = ("<pre><code>Invoke-WebRequest http://SERVER_IP:7331/bundle.tar.gz -OutFile \"$env:TEMP\\sentinel.tar.gz\"\n"
               "tar -xz -f \"$env:TEMP\\sentinel.tar.gz\" -C \"$env:TEMP\"\n"
               "Set-Location \"$env:TEMP\\sentinel\"\n"
               ".\\install.ps1 -Server http://SERVER_IP:7331 -Token YOUR_TOKEN\n\n"
               "# Service management:\nGet-Service SentinelAgent # check status\nRestart-Service SentinelAgent\n# Logs: C:\\ProgramData\\Sentinel\\sentinel-agent.log</code></pre>"
               "<p>Note: For best results install NSSM first (https://nssm.cc). The installer falls back to sc.exe if NSSM is not found.</p>")

    linux = ("<pre><code>curl -s http://SERVER_IP:7331/bundle.tar.gz | tar -xz -C /tmp\n"
             "sudo bash /tmp/sentinel/install.sh --server http://SERVER_IP:7331 --token YOUR_TOKEN\n\n"
             "# Service management:\n"
             "sudo systemctl status sentinel-agent\n"
             "sudo systemctl restart sentinel-agent\n"
             "sudo journalctl -u sentinel-agent -f # live logs\n\n"
             "# Installs to: /opt/sentinel/\nConfig: /etc/sentinel/agent_config.json</code></pre>")
    ...
    troubleshoot = ("<h3>Agent not connecting</h3>"
                    "<ul>"
                    "<li>Check the server is running: <code>curl http://SERVER_IP:7331/health</code></li>"
                    ...
                    "<li>Check agent logs (see platform-specific paths above)</li>"
                    ...
                    "<li>Trigger manual retry: sudo systemctl restart sentinel-agent (Linux) or restart the service</li>"
                    "</ul>"
                    "<h3>Bundle download fails</h3>"
                    "<ul>"
                    "<li>macOS: do not use --overwrite flag, BSD tar does not support it</li>"
                    "<li>Python version error: ensure python3 --version is 3.11 or later</li>"
