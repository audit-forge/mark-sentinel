...
def build(root: pathlib.Path) -> bytes:
    ...
    protected_files = (
        ...
        "<h3>Setting Up Protected Files — Complete Walkthrough</h3>"

        "<p>This section walks through everything you need to do to start using Protected "
        "Files monitoring, from adding your first protected path to reviewing alerts. "
        "No technical background is required — each step explains what to do and why.</p>"

        "<h4>What you'll need first</h4>"
        "<ul>"
        "<li>The <b>Arckon agent</b> installed on at least one device (macOS, Linux, or Windows)</li>"
        "<li>On macOS: the <b>ES Collector</b> installed (see the macOS section above for MDM or manual install)</li>"
        "<li>On Linux: <b>auditd</b> installed (the agent installer does this automatically)</li>"
        "<li>On Windows: nothing extra — the agent runs as admin and Event Log access is automatic</li>"
        "<li>Access to the <b>Arckon dashboard</b> at your server URL</li>"
        "</ul>"
        "<p>If the prerequisites aren't met on a device, the agent will still work for all "
        "other Arckon features — Protected Files monitoring just won't be active on that "
        "specific device until the prerequisites are in place.</p>"

        "<h4>Step 1: Open the Protected Files page</h4>"
        "<ol>"
        "<li>Log in to the Arckon dashboard</li>"
        "<li>In the left sidebar, scroll to the <b>Security</b> section</li>"
        "<li>Click <b>Protected Files</b> (the lock icon)</li>"
        "<li>You'll see four cards at the top: <b>Protected Paths</b>, <b>Access Events</b>, "
        "<b>Unreviewed</b>, and <b>Chain Integrity</b> — all showing zero or ✓ initially</li>"
        "</ol>"

        "<h4>Step 2: Add your first protected path</h4>"
        "<p>A <b>protected path</b> is a file or directory that you want to monitor. When "
        "an AI tool (like Claude, Cursor, Copilot, or Ollama) accesses it, Arckon records "
        "the event and can send you an alert.</p>"
        "<ol>"
        "<li>Click the <b>+ Add Path</b> button (top right of the Protected Paths panel)</li>"
        "<li>A form appears with four fields:</li>"
        "</ol>"

        "<table style='width:100%;border-collapse:collapse;font-size:12px;margin:10px 0'>"
        "<tr style='background:#f3f4f6'>"
        "<th style='padding:8px;text-align:left;border:1px solid #e5e7eb'>Field</th>"
        "<th style='padding:8px;text-align:left;border:1px solid #e5e7eb'>What to enter</th>"
        "<th style='padding:8px;text-align:left;border:1px solid #e5e7eb'>Example</th>"
        "</tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>Path</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>The absolute path of the file or directory to protect. Must start with <code>/</code> (macOS/Linux) or a drive letter (Windows). Relative paths like <code>./secrets</code> are not accepted.</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'><code>/etc/secrets</code><br><code>/Users/keith/.ssh</code><br><code>C:\\\\Company\\\\Payroll</code></td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>Device</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Which device(s) to monitor this path on. Choose a specific device from the dropdown, or select <b>All devices</b> to apply the policy to every device in your fleet.</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>All devices<br>or<br>Keiths-MacBook-Pro</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>Actions</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Which file operations to monitor. <b>Read, Write, Open</b> (default) catches everything. Choose <b>Read only</b> if you only care about AI tools reading sensitive files. Choose <b>Write only</b> if you only care about AI tools modifying files.</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Read, Write, Open (recommended)</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>Recursive</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>If checked (default), monitors all files inside the directory and its subdirectories. If unchecked, only monitors the exact path you entered. <b>Leave this checked</b> for directories — you usually want to protect everything inside them.</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>✅ Checked (for directories)<br>⬜ Unchecked (for single files)</td></tr>"
        "</table>"

        "<ol start='3'>"
        "<li>Click <b>Add</b></li>"
        "</ol>"
        "<p>The path is now protected. The server automatically pushes the policy to the "
        "agent(s) — they receive it within <b>15 seconds</b> on their next command poll. "
        "You'll see the path appear in the Protected Paths list, and the count on the "
        "<b>Protected Paths</b> card will update.</p>"

        "<h4>Real-world examples — what to protect</h4>"

        "<p><b>Example 1: Protect SSH keys on all Macs</b></p>"
        "<p>SSH private keys are highly sensitive — if an AI tool reads them, it could "
        "exfiltrate them or use them to access your servers. To protect them:</p>"
        "<ul>"
        "<li>Path: <code>/Users</code> with Recursive checked</li>"
        "<li>Device: All devices</li>"
        "<li>Actions: Read, Write, Open</li>"
        "</ul>"
        "<p><i>Note:</i> This will monitor all files under <code>/Users</code>, which may "
        "generate many events. For a more targeted approach, protect specific user "
        "directories: <code>/Users/keith/.ssh</code>, <code>/Users/jane/.ssh</code>, etc.</p>"

        "<p><b>Example 2: Protect a company secrets directory on Linux servers</b></p>"
        "<ul>"
        "<li>Path: <code>/opt/company/secrets</code> with Recursive checked</li>"
        "<li>Device: your Linux server's hostname</li>"
        "<li>Actions: Read, Write, Open</li>"
        "</ul>"

        "<p><b>Example 3: Protect a Windows payroll folder</b></p>"
        "<ul>"
        "<li>Path: <code>C:\\\\Company\\\\Payroll</code> with Recursive checked</li>"
        "<li>Device: your Windows device's hostname</li>"
        "<li>Actions: Read, Write, Open</li>"
        "</ul>"

        "<p><b>Example 4: Detect when an AI tool modifies configuration files</b></p>"
        "<p>If you're concerned about an AI tool changing your system configuration:</p>"
        "<ul>"
        "<li>Path: <code>/etc</code> with Recursive checked</li>"
        "<li>Device: All devices</li>"
        "<li>Actions: <b>Write only</b> (only alerts when an AI tool writes/changes a file)</li>"
        "</ul>"

        "<p><b>Example 5: Monitor server logins by AI tools</b></p>"
        "<p>Server login events (SSH on macOS/Linux, RDP on Windows) are automatically "
        "monitored when you add any protected path — you don't need to add a special "
        "\"login\" path. When an AI process initiates an SSH or RDP login, it appears in "
        "the Access Events list with the action <code>login</code>.</p>"

        "<h4>Step 3: Configure alert notifications (so you get told)</h4>"
        "<p>By default, Protected Files events are recorded in the dashboard but <b>no "
        "notifications are sent</b> until you configure at least one alert channel. "
        "To get alerted when an AI tool accesses a protected file:</p>"
        "<ol>"
        "<li>In the left sidebar, click <b>Settings</b> (the gear icon)</li>"
        "<li>Scroll to the <b>Alerts &amp; Notifications</b> section</li>"
        "<li>Configure at least one channel — here are the options:</li>"
        "</ol>"

        "<table style='width:100%;border-collapse:collapse;font-size:12px;margin:10px 0'>"
        "<tr style='background:#f3f4f6'>"
        "<th style='padding:8px;text-align:left;border:1px solid #e5e7eb'>Channel</th>"
        "<th style='padding:8px;text-align:left;border:1px solid #e5e7eb'>How to set up</th>"
        "<th style='padding:8px;text-align:left;border:1px solid #e5e7eb'>Best for</th>"
        "</tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>Slack</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Paste your Slack webhook URL (<code>https://hooks.slack.com/services/...</code>). Create one at Slack → your channel → Integrations → Incoming Webhooks.</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Teams using Slack for real-time alerts</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>Microsoft Teams</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Paste your Teams webhook URL. Create one at Teams → your channel → Connectors → Incoming Webhook.</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Teams using Microsoft Teams</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>Google Chat</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Paste your Google Chat webhook URL. Create one in Google Chat → your space → Apps &amp; integrations → Webhooks.</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Teams using Google Workspace</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>Email</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Enter SMTP server, port, username, password, from address, and to address. Use port 587 with TLS (recommended) or port 465 with SSL.</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Email-based alerting, compliance archives</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>Webhook</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Paste any HTTPS webhook URL. Arckon sends a JSON POST with the event details. Use this to integrate with custom systems.</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Custom integrations, SOAR platforms</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>PSA / Ticketing</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Configure ConnectWise, Autotask, HaloPSA, or Jira. Auto-creates a ticket when an AI tool accesses a protected file.</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>MSPs, IT ticketing workflows</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>Notion</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Configure a Notion integration token and database. Creates a Notion page per event.</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Documentation, audit trail in Notion</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>SIEM</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Configure Splunk, Microsoft Sentinel, Elastic, QRadar, or Exabeam. Forwards events to your SIEM.</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Enterprise security teams, compliance</td></tr>"
        "</table>"

        "<ol start='4'>"
        "<li>After configuring a channel, click <b>Test</b> to send a test alert and verify it works</li>"
        "<li>Click <b>Save</b></li>"
        "</ol>"

        "<p>The <b>AI accessed protected file</b> alert trigger is <b>enabled by default</b> — "
        "you don't need to toggle anything extra. As soon as a channel is configured, "
        "alerts will fire when an AI tool accesses a protected path.</p>"

        "<p><b>How alert deduplication works:</b> Alerts are deduplicated per "
        "(device, path, AI process) with a <b>24-hour cooldown</b>. This means if Claude "
        "reads <code>/etc/secrets</code> 50 times in an hour, you get <b>one</b> alert, "
        "not 50. If Cursor then writes to <code>/etc/secrets</code>, that's a different "
        "(process, action) so you get a second alert. This prevents alert fatigue while "
        "still catching every distinct access pattern.</p>"

        "<h4>Step 4: Review access events</h4>"
        "<p>When an AI tool accesses a protected file, the event appears in the "
        "<b>Access Events</b> panel on the Protected Files page. Here's how to read it:</p>"

        "<p>Each event card shows:</p>"
        "<ul>"
        "<li><b>Action badge</b> (colored, left side) — <span style='color:#CA8A04'>READ</span> (yellow), "
        "<span style='color:#DC2626'>WRITE</span> (red), <span style='color:#4F46E5'>OPEN</span> (indigo), "
        "RENAME, UNLINK, or <span style='color:#4F46E5'>LOGIN</span></li>"
        "<li><b>Process</b> — the AI tool that triggered the event (e.g. <code>claude</code>, "
        "<code>cursor</code>, <code>ollama</code>, <code>copilot</code>)</li>"
        "<li><b>PID</b> — the process ID (for troubleshooting — you can look up the exact process)</li>"
        "<li><b>Path</b> — the protected file that was accessed (monospace font)</li>"
        "<li><b>Device</b> — which endpoint the access occurred on</li>"
        "<li><b>Platform</b> — macOS, Linux, or Windows</li>"
        "<li><b>Source</b> — which collector detected it (<code>esf</code> = macOS Endpoint Security, "
        "<code>auditd</code> = Linux audit, <code>etw</code> = Windows Event Log)</li>"
        "<li><b>Timestamp</b> — when the access occurred (date + time)</li>"
        "</ul>"

        "<p><b>To mark an event as reviewed:</b></p>"
        "<ol>"
        "<li>Click the <b>Review</b> button on the event card</li>"
        "<li>The event fades to 55% opacity and the button changes to \"Reviewed\"</li>"
        "<li>The unreviewed count badge on the sidebar decreases</li>"
        "</ol>"

        "<p><b>To mark all events as reviewed at once:</b></p>"
        "<ol>"
        "<li>Click the <b>Mark all reviewed</b> button at the top of the Access Events panel</li>"
        "<li>All events are marked as reviewed and the sidebar badge disappears</li>"
        "</ol>"

        "<p><b>Filtering events:</b> The Access Events panel shows the 100 most recent events. "
        "To see events for a specific device, use the API endpoint "
        "<code>/api/access-events?device=DEVICE_ID</code> or filter by unreviewed only with "
        "<code>/api/access-events?unreviewed=1</code>.</p>"

        "<h4>Step 5: Check the Chain Integrity indicator</h4>"
        "<p>The <b>Chain Integrity</b> card (top right) shows <b>✓</b> (green) when the "
        "access event log is intact, or <b>⚠</b> (red) if tampering is detected.</p>"
        "<p>All access events are stored in a <b>SHA-256 hash chain</b> — each event's hash "
        "includes the previous event's hash. This means if someone modifies any event in the "
        "database (e.g. trying to delete evidence of an AI tool accessing a file), the chain "
        "breaks for all subsequent events and the indicator turns red.</p>"
        "<p>If you ever see a red ⚠, it means the event log has been tampered with — contact "
        "your security team immediately and investigate who had access to the database.</p>"

        "<h4>Step 6: Review the Policy Change Audit Log</h4>"
        "<p>Scroll to the bottom of the Protected Files page to see the <b>Policy Change "
        "Audit Log</b>. This records every change to the protected-paths policy:</p>"
        "<ul>"
        "<li><b>UPSERT</b> (green) — a path was added or updated</li>"
        "<li><b>REMOVE</b> (red) — a path was removed</li>"
        "</ul>"
        "<p>Each entry shows who made the change, when, and which path was affected. This "
        "satisfies FedRAMP CM-6 (configuration management) — you have a complete record of "
        "who changed what, and when.</p>"

        "<h4>Removing a protected path</h4>"
        "<p>If you no longer want to monitor a path:</p>"
        "<ol>"
        "<li>Find the path in the Protected Paths list</li>"
        "<li>Click the <b>Remove</b> button on the right side of the path card</li>"
        "<li>Confirm the removal in the dialog</li>"
        "</ol>"
        "<p>The path is removed immediately and the updated policy is pushed to the agent(s) "
        "within 15 seconds. The removal is recorded in the Policy Change Audit Log.</p>"

        "<h4>Updating a protected path's settings</h4>"
        "<p>To change which actions are monitored or toggle recursive monitoring for an "
        "existing path, use the dashboard API:</p>"
        "<pre><code>POST /api/protected-paths/&lt;id&gt;/update\n"
        "Content-Type: application/json\n\n"
        "{\"actions\": \"read\", \"recursive\": false}</code></pre>"
        "<p>This is useful for fine-tuning a path that's generating too many events — "
        "e.g. switching from \"Read, Write, Open\" to \"Write only\" to reduce noise.</p>"

        "<h4>Which AI tools are detected?</h4>"
        "<p>Arckon monitors for the following AI/LLM tools (case-insensitive, matches "
        "process name or signing identity):</p>"
        "<table style='width:100%;border-collapse:collapse;font-size:12px;margin:10px 0'>"
        "<tr style='background:#f3f4f6'>"
        "<th style='padding:6px;text-align:left;border:1px solid #e5e7eb'>Category</th>"
        "<th style='padding:6px;text-align:left;border:1px solid #e5e7eb'>Tools detected</th>"
        "</tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>AI coding assistants</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Claude, Cursor, GitHub Copilot, Continue, Codeium, Aider</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>Local LLM runners</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Ollama, LM Studio, vLLM, LocalAI, llama-server, llama.cpp, KoboldCPP, Jan</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>Image generation</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>ComfyUI, Stable Diffusion WebUI, TabbyML</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>SaaS AI clients</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>ChatGPT, Gemini, Poe, Perplexity, Grok</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><b>MCP servers</b></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>mcp-server, FastMCP, any process with <code>@modelcontextprotocol</code> in its path</td></tr>"
        "</table>"
        "<p>If an AI tool you use isn't on this list, contact support — we can add it. "
        "The detection list is regularly updated as new AI tools emerge.</p>"

        "<h4>What happens behind the scenes (technical details)</h4>"
        "<p>For those who want to understand the full flow, here's exactly what happens "
        "from the moment you click \"Add\" to the moment you see an alert:</p>"
        "<ol>"
        "<li><b>You click Add</b> → the dashboard sends a POST to <code>/api/protected-paths</code> "
        "with the path, device, actions, and recursive flag</li>"
        "<li><b>Server stores the policy</b> → the path is encrypted with AES-256-GCM and "
        "stored in the <code>protected_paths</code> table; a record is written to the "
        "<code>protected_paths_audit</code> table (who, when, what)</li>"
        "<li><b>Server pushes the policy to the agent</b> → a <code>set_protected_paths</code> "
        "command is enqueued for the target device(s)</li>"
        "<li><b>Agent receives the policy</b> → on its next command poll (within 15 seconds), "
        "the agent receives the command, updates its local config, and starts (or updates) "
        "the platform collector</li>"
        "<li><b>Collector begins monitoring</b> → the platform-specific collector "
        "(ES framework on macOS, auditd on Linux, Event Log on Windows) starts watching "
        "for file access events on the protected paths</li>"
        "<li><b>AI tool accesses a protected file</b> → the OS generates an audit event; "
        "the collector captures it and checks if the accessing process is an AI tool</li>"
        "<li><b>Event is queued and reported</b> → the agent batches the event and POSTs it "
        "to <code>/api/agent/access-events</code> on the next scan cycle (or immediately for "
        "high-severity events)</li>"
        "<li><b>Server stores the event</b> → the event is appended to the tamper-evident "
        "SHA-256 hash chain in the <code>access_events</code> table</li>"
        "<li><b>Alert fires</b> → if the <code>ai_accessed_protected_file</code> trigger is "
        "enabled (default: yes) and the (device, path, process) combination hasn't alerted "
        "in the last 24 hours, the alert is dispatched to all configured channels</li>"
        "<li><b>Event appears in dashboard</b> → the access event shows up in the Access "
        "Events panel, the unreviewed badge increments, and the Chain Integrity indicator "
        "stays green (✓)</li>"
        "</ol>"
        "<p>The entire flow — from AI tool touching a file to you seeing the event — typically "
        "takes less than 30 seconds.</p>"

        "<h4>Troubleshooting: No events appearing</h4>"
        "<p>If you've added protected paths but no events are showing up, check these "
        "things in order:</p>"
        "<ol>"
        "<li><b>Is the agent running?</b> Check the Fleet page — the device should show a "
        "recent \"last seen\" time. If the agent is offline, events can't be collected.</li>"
        "<li><b>Are the prerequisites met?</b> On macOS, verify the ES Collector is running "
        "(<code>pgrep -fl arckon-es-collector</code>). On Linux, verify auditd is running "
        "(<code>sudo systemctl status auditd</code>). On Windows, verify the agent service "
        "is running as admin.</li>"
        "<li><b>Is an AI tool actually accessing the file?</b> Events only fire when an AI "
        "tool (Claude, Cursor, etc.) accesses the file — not when any process does. Try "
        "opening the protected file with a detected AI tool to test.</li>"
        "<li><b>Is the path correct?</b> The path must be absolute and must exist on the "
        "target device. Check that the path you entered matches the actual file location "
        "on the device. On macOS, paths are canonicalized (symlinks resolved) — "
        "<code>/etc</code> becomes <code>/private/etc</code>.</li>"
        "<li><b>Check the agent log:</b> <code>tail -50 /var/log/arckon-agent.log</code> "
        "(macOS/Linux) or check the Windows Event Viewer for the ArckonAgent service. "
        "Look for messages about \"Protected-files collector\" or \"access event\".</li>"
        "</ol>"

        "<h4>Troubleshooting: Too many events (alert fatigue)</h4>"
        "<p>If you're getting too many alerts:</p>"
        "<ol>"
        "<li><b>Narrow the actions filter</b> — change from \"Read, Write, Open\" to "
        "\"Write only\" if you only care about modifications</li>"
        "<li><b>Be more specific with paths</b> — instead of protecting <code>/etc</code> "
        "(which catches everything), protect specific files like <code>/etc/passwd</code> "
        "or <code>/etc/shadow</code></li>"
        "<li><b>Turn off recursive</b> — if you only care about a specific file, not a "
        "directory tree, uncheck the Recursive option</li>"
        "<li><b>Rely on the 24-hour dedup</b> — the same (device, path, process) only "
        "alerts once per 24 hours, so repeated access by the same tool won't spam you</li>"
        "</ol>"
        ...
        "<p>The macOS collector uses Apple's <b>Endpoint Security framework</b> — the same "
        "kernel-level monitoring API used by CrowdStrike, SentinelOne, and other enterprise "
        "EDR tools. It requires a small, signed, notarized helper app called "
        "<code>ArckonESCollector.app</code> that runs as a root LaunchDaemon and streams "
        "file-access events to the Arckon agent.</p>"

        "<p>There are two ways to install it: <b>MDM deployment</b> (for managed fleets — "
        "fully automatic, no user interaction) or <b>manual install</b> (for standalone Macs "
        "— one manual step in System Settings).</p>"

        "<h4>What gets installed</h4>"
        "<table style='width:100%;border-collapse:collapse;font-size:12px;margin-bottom:12px'>"
        "<tr style='background:#f3f4f6'>"
        "<th style='padding:8px;text-align:left;border:1px solid #e5e7eb'>Component</th>"
        "<th style='padding:8px;text-align:left;border:1px solid #e5e7eb'>Location</th>"
        "<th style='padding:8px;text-align:left;border:1px solid #e5e7eb'>Purpose</th>"
        "</tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'><code>ArckonESCollector.app</code></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'><code>/Library/Arckon/</code></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Signed, notarized ES helper binary</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'>LaunchDaemon plist</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'><code>/Library/LaunchDaemons/ai.mfdynamics.arckon-es-collector.plist</code></td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Auto-starts the ES helper on boot, keeps it alive</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'>Full Disk Access (TCC)</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>System Settings → Privacy &amp; Security</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Apple permission required for ES framework access</td></tr>"
        "</table>"

        "<h4>Option A: MDM Deployment (Recommended — Zero User Interaction)</h4>"
        "<p>For Macs managed by Jamf, Kandji, Intune, Mosyle, or Apple Business Manager, "
        "the entire deployment is silent. The PPPC (Privacy Preferences Policy Control) "
        "profile pre-grants Full Disk Access so there's no manual System Settings step.</p>"

        "<p><b>You need two artifacts</b> (both signed and notarized by M. F. Dynamics LLC):</p>"
        "<ul>"
        "<li><code>Arckon-ES-Collector-1.0.34.pkg</code> — the .pkg installer that deploys the ES helper + LaunchDaemon</li>"
        "<li><code>arckon-es-collector.pppc.mobileconfig</code> — the MDM configuration profile that pre-grants Full Disk Access</li>"
        "</ul>"

        "<p><b>Deploy in this exact order</b> (the PPPC profile must be installed before or "
        "with the .pkg so Full Disk Access is already granted when the daemon first starts):</p>"

        "<h5>Step 1: Upload the PPPC Profile to your MDM</h5>"
        "<p>This profile tells macOS to silently grant Full Disk Access to the Arckon ES "
        "helper. Without it, each user would have to manually click through System Settings.</p>"

        "<p><b>Jamf Pro:</b></p>"
        "<ol>"
        "<li>Log in to Jamf Pro → <b>Computers</b> → <b>Configuration Profiles</b> → <b>New</b></li>"
        "<li>Set the name to <code>Arckon ES Collector — Full Disk Access</code></li>"
        "<li>Set Level to <b>Computer-level</b></li>"
        "<li>Scroll down to <b>Application &amp; Custom Settings</b></li>"
        "<li>Click <b>Upload</b> and select <code>arckon-es-collector.pppc.mobileconfig</code></li>"
        "<li>Click the <b>Scope</b> tab and select the target devices or device group</li>"
        "<li>Click <b>Save</b> → the profile deploys on next MDM check-in</li>"
        "</ol>"

        "<p><b>Kandji:</b></p>"
        "<ol>"
        "<li>Log in to Kandji → <b>Library</b> → <b>New Profile</b> → <b>Custom Profile</b></li>"
        "<li>Set the name to <code>Arckon ES Collector — Full Disk Access</code></li>"
        "<li>Upload <code>arckon-es-collector.pppc.mobileconfig</code></li>"
        "<li>Click <b>Scope</b> and select target devices</li>"
        "<li>Click <b>Save &amp; Deploy</b></li>"
        "</ol>"

        "<p><b>Microsoft Intune:</b></p>"
        "<ol>"
        "<li>Log in to Microsoft Intune → <b>Devices</b> → <b>macOS</b> → <b>Configuration profiles</b> → <b>Create profile</b></li>"
        "<li>Platform: <b>macOS</b> → Profile type: <b>Templates</b> → <b>Custom</b></li>"
        "<li>Set the name to <code>Arckon ES Collector — Full Disk Access</code></li>"
        "<li>Under <b>Custom configuration settings</b>, click <b>Upload</b> and select <code>arckon-es-collector.pppc.mobileconfig</code></li>"
        "<li>Click <b>Assignments</b> and select the target device groups</li>"
        "<li>Click <b>Create</b></li>"
        "</ol>"

        "<p><b>Mosyle Fuse:</b></p>"
        "<ol>"
        "<li>Log in to Mosyle Fuse → <b>Management</b> → <b>Profiles</b> → <b>Custom Profile</b></li>"
        "<li>Set the name to <code>Arckon ES Collector — Full Disk Access</code></li>"
        "<li>Upload <code>arckon-es-collector.pppc.mobileconfig</code></li>"
        "<li>Click <b>Scope</b> and select target devices</li>"
        "<li>Click <b>Save &amp; Deploy</b></li>"
        ...
        "<p><b>Apple Business Manager (ABM) / Apple School Manager (ASM):</b></p>"
        "<p>If your MDM is linked to ABM/ASM, the PPPC profile is pushed automatically to "
        "all enrolled devices through your MDM vendor's ABM integration. Follow your MDM "
        "vendor's instructions for uploading a custom configuration profile — the steps are "
        "the same as above for your specific MDM.</p>"

        "<h5>Step 2: Deploy the .pkg Installer via your MDM</h5>"
        "<p>This installs the ES helper app and its LaunchDaemon on each Mac.</p>"

        "<p><b>Jamf Pro:</b></p>"
        "<ol>"
        "<li><b>Computer Management</b> → <b>Packages</b> → <b>New</b> → <b>Upload</b></li>"
        "<li>Select <code>Arckon-ES-Collector-1.0.34.pkg</code> and upload it</li>"
        "<li>Go to <b>Computers</b> → <b>Policies</b> → <b>New</b></li>"
        "<li>Set Trigger to <b>Enrollment</b> (installs on new enrollments) or <b>Recurring check-in</b> (installs on next check-in for existing devices)</li>"
        "<li>Under <b>Packages</b>, select the Arckon ES Collector package → Action: <b>Install</b></li>"
        "<li>Click <b>Scope</b> and select the same devices as the PPPC profile</li>"
        "<li>Click <b>Save</b></li>"
        "</ol>"

        "<p><b>Kandji:</b></p>"
        "<ol>"
        "<li><b>Library</b> → <b>New App</b> → <b>Custom App</b></li>"
        "<li>Upload <code>Arckon-ES-Collector-1.0.34.pkg</code></li>"
        "<li>Set Distribution Method to <b>Auto-install</b></li>"
        "<li>Click <b>Scope</b> and select the same devices as the PPPC profile</li>"
        "<li>Click <b>Save &amp; Deploy</b></li>"
        "</ol>"
        ...
        "<p><b>Microsoft Intune:</b></p>"
        "<ol>"
        "<li><b>Apps</b> → <b>macOS</b> → <b>Add</b> → <b>Line-of-business app</b></li>"
        "<li>Upload <code>Arckon-ES-Collector-1.0.34.pkg</code></li>"
        "<li>Fill in app details (name, publisher, etc.)</li>"
        "<li>Click <b>Assignments</b> and select the same device groups as the PPPC profile</li>"
        "<li>Click <b>Create</b></li>"
        "</ol>"

        "<p><b>Mosyle Fuse:</b></p>"
        "<ol>"
        "<li><b>Management</b> → <b>Apps</b> → <b>Custom App</b></li>"
        "<li>Upload <code>Arckon-ES-Collector-1.0.34.pkg</code></li>"
        "<li>Set Auto-install to <b>Enabled</b></li>"
        "<li>Click <b>Scope</b> and select the same devices as the PPPC profile</li>"
        "<li>Click <b>Save &amp; Deploy</b></li>"
        "</ol>"

        "<h5>Step 3: Deploy the Arckon Agent</h5>"
        "<p>Deploy the Arckon agent to the same devices using your MDM's package deployment "
        "(same method as Step 2, using the agent installer package), or users can run the "
        "standard install command:</p>"
        "<pre><code>curl -sSL https://arckon.riskraven.ai/install.sh | sudo bash -s -- --server https://arckon.riskraven.ai --token YOUR_TOKEN</code></pre>"
        "<p>The agent installer will detect the already-installed ES helper and automatically "
        "connect to it — no special configuration needed.</p>"

        "<h5>Step 4: Verify (optional — confirms everything is working)</h5>"
        "<p>On any managed Mac, run these commands in Terminal to verify the deployment:</p>"
        "<pre><code># Check the LaunchDaemon is loaded and running\n"
        "sudo launchctl print system/ai.mfdynamics.arckon-es-collector | head -5\n\n"
        "# Check the ES helper process is running\n"
        "pgrep -fl arckon-es-collector\n\n"
        "# Check the PPPC profile was installed by MDM\n"
        "profiles show -type configuration | grep arckon\n\n"
        "# Check the Unix socket exists (agent connects here)\n"
        "ls -la /var/run/arckon-es-collector.sock\n\n"
        "# Check the agent is connected to the ES helper\n"
        "tail -20 /var/log/arckon-agent.log | grep -i \"ES.*connected\"</code></pre>"
        "<p>If all commands return results, Protected Files monitoring is fully operational "
        "on that Mac. Add protected paths in the dashboard and events will start flowing.</p>"

        "<h5>What happens on each managed Mac (automatically, no user prompts):</h5>"
        "<ol>"
        "<li><b>MDM pushes the PPPC profile</b> → Full Disk Access is silently pre-granted for <code>ai.mfdynamics.arckon.agent</code></li>"
        "<li><b>MDM pushes the .pkg</b> → <code>ArckonESCollector.app</code> installed to <code>/Library/Arckon/</code>, LaunchDaemon loaded, ES daemon starts</li>"
        "<li><b>MDM pushes (or user installs) the Arckon agent</b> → agent connects to the ES daemon's Unix socket, begins monitoring</li>"
        "<li><b>Admin adds protected paths in dashboard</b> → policy pushed to agent within 15 seconds, monitoring active</li>"
        "</ol>"
        "<p><b>No user interaction required at any point.</b> The entire deployment is silent.</p>"

        "<h4>Option B: Manual Install (Standalone / Non-Managed Macs)</h4>"
        "<p>For Macs that are <b>not</b> enrolled in MDM, Apple requires one manual step in "
        "System Settings (TCC permissions cannot be granted by a script on non-managed Macs "
        "— this is a deliberate Apple security boundary).</p>"
        "<ol>"
        "<li>Install the .pkg:<br><code>sudo installer -pkg Arckon-ES-Collector-1.0.34.pkg -target /</code></li>"
        "<li>Open <b>System Settings</b> → <b>Privacy &amp; Security</b> → <b>Full Disk Access</b></li>"
        "<li>Click the <b>+</b> button</li>"
        "<li>Navigate to <code>/Library/Arckon/ArckonESCollector.app</code> and select it</li>"
        "<li>Make sure the toggle next to <code>arckon-es-collector</code> is <b>ON</b></li>"
        "<li>Install the Arckon agent:<br><code>curl -sSL https://arckon.riskraven.ai/install.sh | sudo bash -s -- --server https://arckon.riskraven.ai --token YOUR_TOKEN</code></li>"
        "</ol>"
        "<p>If the ES collector is not installed, the agent continues to work normally for "
        "all other Arckon features — Protected Files monitoring is simply not active on "
        "that Mac until the ES helper is installed.</p>"

        "<h4>Troubleshooting macOS Deployment</h4>"

        "<p><b>ES daemon won't start (\"not permitted\" error):</b></p>"
        "<p>This means Full Disk Access was not granted. On MDM-managed Macs, verify the "
        "PPPC profile is installed:</p>"
        "<pre><code>profiles show -type configuration | grep arckon</code></pre>"
        "<p>If no profile appears, the MDM hasn't delivered it yet — check your MDM scope "
        "and wait for the next check-in. On standalone Macs, repeat the manual System "
        "Settings step above.</p>"

        "<p><b>ES daemon running but no events in dashboard:</b></p>"
        "<p>Check that the agent is connected to the ES helper's Unix socket:</p>"
        "<pre><code>ls -la /var/run/arckon-es-collector.sock\n"
        "tail -30 /var/log/arckon-agent.log | grep -i \"ES.*protected.*collector\"</code></pre>"
        "<p>If the socket doesn't exist, the ES daemon isn't running. If the agent log "
        "doesn't mention the ES collector, restart the agent:<br><code>sudo launchctl kickstart -k system/ai.mfdynamics.arckon-agent</code></p>"

        "<p><b>PPPC profile installed but Full Disk Access still not granted:</b></p>"
        "<p>Verify the bundle ID in the PPPC profile matches exactly: "
        "<code>ai.mfdynamics.arckon.agent</code>. If the profile references a different "
        "bundle ID, macOS will not grant the permission. Re-download the profile from "
        "the Arckon dashboard or your deployment package.</p>"

        "<p><b>\"missing entitlement\" error in the ES daemon log:</b></p>"
        "<p>This means the <code>ArckonESCollector.app</code> was not properly signed with "
        "the Endpoint Security entitlement. Re-download the official .pkg from the Arckon "
        "dashboard — do not re-sign or modify the app. The official build is signed by "
        "M. F. Dynamics LLC (Team ID <code>SWRJ6ZV39K</code>) and notarized by Apple.</p>"

        "<h4>Code Signing &amp; Trust Details</h4>"
        "<table style='width:100%;border-collapse:collapse;font-size:12px;margin-bottom:12px'>"
        "<tr style='background:#f3f4f6'>"
        "<th style='padding:6px;text-align:left;border:1px solid #e5e7eb'>Property</th>"
        "<th style='padding:6px;text-align:left;border:1px solid #e5e7eb'>Value</th>"
        "</tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'>Bundle ID</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'><code>ai.mfdynamics.arckon.agent</code></td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'>Team ID</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'><code>SWRJ6ZV39K</code> (M. F. Dynamics LLC)</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'>Signing identity</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Developer ID Application: M. F. Dynamics LLC</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'>Installer identity</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Developer ID Installer: M. F. Dynamics LLC</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'>Notarization</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Apple status: Accepted (stapled ticket included)</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'>Entitlement</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'><code>com.apple.developer.endpoint-security.client</code> (approved by Apple 2026-07-10)</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'>Minimum macOS</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>macOS 12.0 (Monterey) or later</td></tr>"
        "<tr><td style='padding:6px;border:1px solid #e5e7eb'>Architecture</td>"
        "<td style='padding:6px;border:1px solid #e5e7eb'>Universal (arm64 + x86_64)</td></tr>"
        "</table>"

        "<p><b>Code requirement</b> (for custom PPPC profiles or Gatekeeper verification):</p>"
        "<pre><code>identifier \"ai.mfdynamics.arckon.agent\" and anchor apple generic and\n"
        "certificate 1[field.1.2.840.113635.100.6.2.6] /* exists */ and\n"
        "certificate leaf[field.1.2.840.113635.100.6.1.13] /* exists */ and\n"
        "certificate leaf[subject.OU] = SWRJ6ZV39K</code></pre>"

        "<h4>Linux: Prerequisites (Auto-Installed)</h4>"
        "<p>On Linux, the Arckon agent installer (<code>install.sh</code>) automatically "
        "installs <code>auditd</code> (the Linux audit daemon) if it's not already present. "
        "The agent runs as root (required by the installer), which is needed for "
        "<code>auditctl</code> and <code>ausearch</code> access.</p>"
        "<p>Supported package managers: <code>apt-get</code> (Debian/Ubuntu), <code>yum</code> "
        "(RHEL/CentOS), <code>dnf</code> (Fedora/RHEL 8+). No manual steps required.</p>"

        "<h4>Windows: Prerequisites (None Required)</h4>"
        "<p>On Windows, the agent installer (<code>install.ps1</code>) requires admin "
        "privileges (<code>#Requires -RunAsAdministrator</code>), which is all that's needed "
        "for Windows Event Log access (Event ID 4663 for file access, 4624 for logins). "
        "No additional packages or configuration needed — Protected Files monitoring works "
        "immediately after agent install.</p>"
